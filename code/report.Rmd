...
if(mean(!is.na(results$priceMoneyUSDollar)) > 1/3) { # Include this section only if the price money of the player is not missing too often.
  cat('# Price Money\n')
...
if(numberOfMatches > 100) {
  if(!is.nan(rSquared)) {
    ...
    # In the lines 311-580, the if-statements check for every variable whether the estimated coefficient is statistically (p-value < 0.1) and economically (estimated coefficient > 0.005) significant.
    ...
    if(overview$active == 0) { # If not active any more, use past simple.
      ...
      if(isTRUE(lpm$p.value[lpm$term == 'Tournament at Home'] > 0.1 | round(lpm$estimate[lpm$term == 'Tournament at Home'], digits = 2) == 0)) { # Not significant.
